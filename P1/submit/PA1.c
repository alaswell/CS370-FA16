...
// ANSWER: the head of a linked list is a pointer so to access the data you need a pointer to point at that pointer. Otherwise you would just be modifying the stack variable and wouldn't ever actually changed the linked list. 
...
if(current) {
	// head is not null, check if head matches
	if(current->ISBN == ISBN) {
		if(DEBUG) printf("Head matches | incrementing count\n");
		current->CopiesAtCSU++;
		return 1;
	}
	else {
		...
		while(current->next) {
			// compare the ISBN
			if(current->ISBN == ISBN) {
				if(DEBUG) printf("Copy found | incrementing count\n");
				current->CopiesAtCSU++;
				return 1;
			}
			else {
				...
				if(DEBUG) printf("listHead %d\n", (*listHead));
			}
	
		}
	}
	// no copy found | reset current to head
	current = *listHead;
	prev = current;
	if(DEBUG) printf("no copy found | reset\n");
}
...
switch(sortIndx) {
	case 1:
		...
		if(!current->next) {
			// no next so just compare head
			if(current->ISBN < ISBN) {
				if(DEBUG) printf("cur < new\n");
				locFound = 1;
			}
			else {
				if(DEBUG) printf("cur > new | cur: %d new: %d\n", current, newNode);
				if(DEBUG) printf("curISBN: %d newISBN: %d\n", current->ISBN, newNode->ISBN);
			}
		}	
...
if(locFound == 0) {
	if(DEBUG) printf("locFound = 0 | cur %d | new %d\n", current, newNode);
	newNode->next = *listHead;
	(*listHead) = newNode;	// newNode is now head
	if(DEBUG) printf("listHead : %d\n newNode : %d\n newNext : %d\n", (*listHead), newNode, newNode->next);
}
else if(locFound == 1) {
	if(DEBUG) printf("locFound = 1 | cur %d | new %d\n", current, newNode);
	for(int i = 0; i < locFound; i++) {
		prev = current;
		current = current->next;
	}
	prev->next = newNode;
	...
	// walk through LL to position
	for(int i = 0; i < locFound + 1; i++) {
		if(DEBUG) printf("walking : prev %d cur %d head %d\n", prev, current, (*listHead));
		prev = current;
		if(current->next)
			current = current->next;
	}

	// if prev == cur there is no next
	if(prev != current) {
		newNode->next = current;
		prev->next = newNode;
	}
